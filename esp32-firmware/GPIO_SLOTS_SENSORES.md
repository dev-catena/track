...
| 5 | 26 | Chave: HIGH = bateria no slot, LOW = vazio |
| 6 | 27 | Chave: HIGH = bateria no slot, LOW = vazio |

**Nota:** GPIO 34 e 35 são input-only no ESP32. Para inverter a lógica (LOW = ocupado), altere `SENSOR_OCCUPIED_HIGH` para `0` em `main.cpp`.
