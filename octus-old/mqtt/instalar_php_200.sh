...
# Script para instalar PHP no servidor 10.100.0.200
# Execute como: sudo ./instalar_php_200.sh
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Execute: sudo ./instalar_php_200.sh"
    exit 1
fi
...
echo "🔧 Verificando versão do PHP disponível..."
apt search php | grep -E "^php[0-9]+\.[0-9]+" | head -5
