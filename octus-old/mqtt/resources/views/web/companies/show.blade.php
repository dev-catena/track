                    ...
                    <div class="stat-content">
                        <div class="stat-number">
                            @if(isset($company['departments']))
                                {{ count(array_filter($company['departments'], function($dept) { return empty($dept['id_unid_up'] ?? $dept['parent_id'] ?? null); })) }}
                            @else
                                0
                            @endif
                        </div>
                                    ...
                                    @foreach($departments as $department)
                                        <div class="department-card level-{{ $level }}">
                                            ...
                                            <div class="department-meta">
                                                @if(($department['id_unid_up'] ?? $department['parent_id'] ?? null))
                                                    <div class="parent-info">
                                                        <small>
                                                            👆 Subordinado a: 
                                                            @php
                                                                $parentId = $department['id_unid_up'] ?? $department['parent_id'] ?? null;
                                                                $parent = collect($company['departments'])->firstWhere('id', $parentId);
                                                            @endphp
                                                            @if($parent)
