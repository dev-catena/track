...
#define SENSOR_PINS { 32, 33, 34, 35, 36, 39 }
...
#define MQTT_SERVER "10.102.0.103"
...
void postSlotStatusToBackend(const int* slotsOccupied);
...
void loop() {
    if (slotLedOnAt > 0 && (millis() - slotLedOnAt) >= SLOT_LED_DURATION_MS) {
        ...
        if (slotLedActive >= 1 && slotLedActive <= NUM_SLOTS) {
            const int ledPins[NUM_SLOTS] = SLOT_LED_PINS;
            digitalWrite(ledPins[slotLedActive - 1], LOW);
        }
        slotLedActive = 0;
        Serial.println("💡 LED slot apagado após 10s");
    }
...
bool checkinToBackend() {
    ...
    HTTPClient http;
    String url = "http://" + String(BACKEND_SERVER) + ":" + String(BACKEND_PORT) + "/api/devices/checkin";
    http.begin(url);
    ...
    if (command == "led_on") {
        ...
        if (command == "open") {
            if (slot >= 1 && slot <= NUM_SLOTS) {
                ...
                postSlotStatusToBackend(slotsOccupied);
            }
        } else {
            if (slot >= 1 && slot <= NUM_SLOTS) {
                closeSlot(slot);
...
void postSlotStatusToBackend(const int* slotsOccupied) {
    ...
    HTTPClient http;
    String url = "http://" + String(BACKEND_SERVER) + ":" + String(BACKEND_PORT) + "/api/docks/slot-status";
    http.begin(url);
    http.addHeader("Content-Type", "application/json");
...
void openSlot(int slot) {
    const int ledPins[NUM_SLOTS] = SLOT_LED_PINS;
    if (slot >= 1 && slot <= NUM_SLOTS) {
        digitalWrite(ledPins[slot - 1], HIGH);
        slotLedActive = slot;
        if (slotLedOnAt == 0) {
            slotLedOnAt = millis();
        }
